...
server <- function(input, output, session) {
  ...
  get_cov_from_ui <- reactive({
    ...
    mk_cov_df_from_ui(
      ...
      ca = input$demos_ca, 
      comorb = input$demos_comorb, 
      dep = input$demos_dep, 
      t2d = input$demos_t2d,
      duration = input$demos_duration,
      work = input$demos_work,
      verbose = TRUE
    )
  })  
    
  get_knee_outc <- reactive({
    this_outc <- input$knee_outc
 ...
 ggplot1 <- reactive({
  ...
  plotly1 <- reactive({
    ...
    rm(list = "grid_opt_obj") # unused herein, free memory
   ...
   outc_nm <- names(knee_outc_choices_alt)[knee_outc_choices_alt == get_knee_outc()]
   fig <- fig %>% 
     add_trace(
       y = ~ideal_day, 
       name = paste0(
  ...
  output$ui0 <- renderUI({
    ...
    card(
      full_screen = TRUE,
      card_header(paste0("Your current day vs. your 'best' day (", outc_nm, ")")),
      layout_columns(
        col_widths  = c(8, 4),
        box(width = 12,
        ...
        box(width = 12,
            p(
              ...
              strong("'best'", style = "color: #00b0f0"), 
  ...
  #   [ORANGE] light otherwise
    ...
    m_v_lst_strata <- 
      m_and_v_ilrs %>% 
      dplyr::filter(strata_id.x == contraint_strata)
    ...
    if (abs(current_24_diff()) < 1e-3) {
      out_card <- 
        card(
          ...
          layout_columns(
            p(
              "You have now created your lifestyle profile. On the next page, we will show you a visual of what the 'best' 24-hours looks like for your cognitive function, compared to what your current day looks like."
  ...
  observeEvent(input$next1, {
    nav_select(id = "pillset", selected = "Knee outcome")
  })
  ...
  observeEvent(input$next5, {
    nav_select(id = "pillset", selected = "Best day")
  })
